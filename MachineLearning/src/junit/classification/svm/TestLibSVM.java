...
public class TestLibSVM {
	...
	public void testLibSVM() {
		try {
			Dataset data = FileHandler.loadDataset(new File("src/data/iris.tsv"), 4, "\t");
			LibSVM ls = new LibSVM();
			// ls.buildClassifier(data);

			ls.buildClassifier(data);
			data = FileHandler.loadDataset(new File("src/data/iris.tsv"), 4, "\t");
			int t = 0, f = 0;
			for (Instance i : data)
	...
	public void testLibSVMBupa() {
		try {
			...
			Dataset data = FileHandler.loadDataset(new File("src/data/BUPA.tsv"), 6);
			LibSVM ls = new LibSVM();
			// ls.buildClassifier(data);

			ls.buildClassifier(data);
			data = FileHandler.loadDataset(new File("src/data/BUPA.tsv"), 6);
			int t = 0, f = 0;
	...
	public void testLibSVMSparse() {
		try {
			Dataset data = FileHandler.loadSparseDataset(new File("src/data/sparse.tsv"), 0, " ", ":");
			LibSVM ls = new LibSVM();
			ls.buildClassifier(data);
			data = FileHandler.loadSparseDataset(new File("src/data/sparse.tsv"), 0, " ", ":");
			int t = 0, f = 0;
			for (Instance i : data) {
				if (i.classValue().equals(ls.classify(i)))
					t++;
	...
	public void testLibSVMSparse2() {
		try {
			Dataset data = FileHandler.loadSparseDataset(new File("src/data/testdata01"), 0, " ", ":");
