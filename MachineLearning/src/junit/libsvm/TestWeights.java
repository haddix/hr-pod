...
public class TestWeights {
	...
	public void testWeights() {
		try {
			Dataset data = FileHandler.loadSparseDataset(new File("src/data/4gate.txt"), 0, " ", ":");
			LibSVM svm = new LibSVM();
			svm.buildClassifier(data);
			double[] w1 = svm.getWeights();

			data = FileHandler.loadSparseDataset(new File("src/data/4gate_original.txt"), 0, " ", ":");
			svm = new LibSVM();
			svm.buildClassifier(data);
			double[] w2 = svm.getWeights();
			
			data = FileHandler.loadSparseDataset(new File("src/data/4gatesparse.txt"), 0, " ", ":");
			svm = new LibSVM();
			svm.buildClassifier(data);
			double[] w3 = svm.getWeights();
			
			data = FileHandler.loadDataset(new File("src/data/4gatedense.txt"), 0, " ");
			svm = new LibSVM();
			svm.buildClassifier(data);
